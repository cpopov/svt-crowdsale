...
describe('ExampleCrowdSale', function () {
  let token: ExampleToken;
  let vestingVault: ExampleVestingVault;
  let crowdSale: ExampleCrowdSale;
  ...
  const tokenAmount = 1;
  const increaseExampleTokensBy = 5;
  let newExampleTokenAmount = tokenAmount + increaseExampleTokensBy;
  type Vesting = {
  ...
  before(async function () {
    ...
    token = (await deployContract(adminUser, ExampleTokenArtifact, [
      'ExampleToken',
      'ET',
      initialSupply,
      wallet.address,
    ...
    vestingVault = (await deployContract(adminUser, ExampleVestingVaultArtifact, [
      token.address,
    ...
    crowdSale = (await deployContract(adminUser, ExampleCrowdSaleArtifact, [
      pricefeed,
      token.address,
      wallet.address,
      250,
  ...
  it('cannot grant vault controller role if sender is not admin', async () => {
    ...
    const userOneExampleVestingVault = vestingVault.connect(userOne);
    expect(userOneExampleVestingVault.grantRole(role, crowdSale.address)).to.be.reverted;
  });
  ...
  it('can add a new beneficiary to the vesting vault', async () => {
    ...
    let totalExampleTokenAmount: BigNumber = BigNumber.from(0);
    vestings.map((vest) => (totalExampleTokenAmount = totalExampleTokenAmount.add(vest.tokenAmount)));

    expect(totalExampleTokenAmount).to.be.equal(tokenAmount);
  ...
  it('can increase the tokens of an existing beneficiary for an existing release time', async () => {
    expect(await vestingVault.addBeneficiary(userOne.address, releaseTime, increaseExampleTokensBy))
      .to.emit(vestingVault, 'VestingLockedIn')
      .withArgs(userOne.address, releaseTime, increaseExampleTokensBy);

    await token.connect(wallet).transfer(vestingVault.address, increaseExampleTokensBy);

    const vestings = (await vestingVault.vestingFor(userOne.address)) as unknown as Vesting[];
    let totalExampleTokenAmount: BigNumber = BigNumber.from(0);
    vestings.map((vest) => (totalExampleTokenAmount = totalExampleTokenAmount.add(vest.tokenAmount)));

    expect(totalExampleTokenAmount).to.be.equal(newExampleTokenAmount);

    expect(await token.balanceOf(vestingVault.address)).to.be.equal(newExampleTokenAmount);
  });
  ...
  it('can add another vesting to an existing beneficiary for a different release time', async () => {
    ...
    newExampleTokenAmount = newExampleTokenAmount + tokenAmount;
  ...
  it('can release the vesting tokens of a beneficiary', async () => {
    ...
    const userOneExampleVestingVault = vestingVault.connect(userOne);
    expect(await userOneExampleVestingVault.release()).to.emit(userOneExampleVestingVault, 'VestingReleased');

    // Balance of tokens of userOne increases
    expect(await token.balanceOf(userOne.address)).to.equal(newExampleTokenAmount);
  ...
  it('fails to release tokens before release time', async () => {
    ...
    // Add userTwo as a beneficiary in the ExampleVestingVault
  ...
  it('when there are multiple release times, it releases tokens only for which release time is enabled', async () => {
    ...
    // Add another entry for userTwo beneficiary in the ExampleVestingVault
    ...
    const userTwoExampleVestingVault = vestingVault.connect(userTwo);
    expect(await userTwoExampleVestingVault.release()).to.emit(userTwoExampleVestingVault, 'VestingReleased');
  ...
  it('user can not release tokens not belonging to him', async () => {
    ...
    // Add adminUser as a beneficiary in the ExampleVestingVault
    ...
    // Admin user has tokenAmount stored in the ExampleVestingVault
